(function (name, factory) {
	...
	Factory.prototype = {
		constructor: Factory, setOptions: function (options) {
			...
			var dir = Math.round(random(1, 9)) > 5 ? "right" : "left", stop = 1000, hide = 300, min = 0 - hide,
				max = this._width + hide, movex = 0, movey = 0, startx = dir === "right" ? min : max,
				starty = Math.round(random(this._scroll, this._scroll + this._height));
			if (/^(top|min)$/i.test(this._options.verticalPosition)) {
				starty = this._scroll + hide;
			} else if (/^(middle|center)$/i.test(this._options.verticalPosition)) {
				starty = this._scroll + this._height / 2;
